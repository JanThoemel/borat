...
for k=1:length(y)
    for j=1:length(x)
        ...
        if Rk>R
            ...
            n(k,j)=sqrt(2-(Rk/R)^2);
            nx(k,j)=(-2*X(k,j)/R^2)/(2*sqrt(2-(Rk/R)^2));
            ny(k,j)=(-2*Y(k,j)/R^2)/(2*sqrt(2-(Rk/R)^2));
        end
    end
end
...
y_0_vec=linspace(1.99,-1.99,n_ray);

itpo=zeros(3,200,n_ray);

for k=1:length(y_0_vec)
...
% 
% output_fname = 'Luneburg2D_domain.szplt';
% output_fname_rays = 'Luneburg_Rays_2D.szplt';
% 
% 

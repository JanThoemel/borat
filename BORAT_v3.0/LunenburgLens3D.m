    ...
    for j=1:length(x)
        for l=1:length(z)
            ...
            if Rk>R
                ...
                n(k,j,l)=sqrt(2-(Rk/R)^2);
                nx(k,j,l)=(-2*X(k,j,l)/R^2)/(2*sqrt(2-(Rk/R)^2));
                ny(k,j,l)=(-2*Y(k,j,l)/R^2)/(2*sqrt(2-(Rk/R)^2));
                nz(k,j,l)=(-2*Z(k,j,l)/R^2)/(2*sqrt(2-(Rk/R)^2));
...
itpo=zeros(4,nResemple,n_radius*n_alpha);
...
for k=1:length(R0)
    for i=1:length(alpha)
        ...
        x_0=-4;
        y_0=R0(k)*cos(alpha(i)*pi/180);
        z_0=R0(k)*sin(alpha(i)*pi/180);
...
% 
% output_fname = 'Luneburg3D_domain.szplt';
% output_fname_rays = 'Luneburg_Rays_3D.szplt';
% 
% 
...
function [n,nx,ny,nz] = refractive(x,y,z)
    ...
    if Rk>R
        ...
        n=sqrt(2-(Rk/R)^2);
        nx=(-2*x/R^2)/(2*sqrt(2-(Rk/R)^2));
        ny=(-2*y/R^2)/(2*sqrt(2-(Rk/R)^2));
        nz=(-2*z/R^2)/(2*sqrt(2-(Rk/R)^2));
